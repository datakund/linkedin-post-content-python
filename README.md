...
A python library that uses browser automation to post content on linkedin feed
...
```javascript
from linkedin-content-uploader import *
```

### Login

To post content on linkedin first we need to login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To post content on linkedin feed we use ``upload_content`` method.
It requires **content** as input parameter

```javascript
linkedin.upload_content(content="paste_text_content_here")
```
